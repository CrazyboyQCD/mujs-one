...
#[no_mangle]
pub unsafe extern "C" fn js_fmtexp(mut p: *mut libc::c_char, mut e: i32) {
    ...
    *fresh16 = 'e' as i32 as libc::c_char;
    ...
    while e != 0 {
        let fresh19 = i;
        i += 1;
        se[fresh19 as usize] = (e % 10 + '0' as i32) as libc::c_char;
        e /= 10;
    }
    while i < 1 {
        let fresh20 = i;
...
unsafe extern "C" fn cached_power(mut k: i32) -> diy_fp_t {
    let mut res: diy_fp_t = diy_fp_t { f: 0, e: 0 };
    let mut index: i32 = 343 as i32 + k;
    res.f = POWERS_TEN[index as usize];
    res.e = POWERS_TEN_E[index as usize];
    res
}
unsafe extern "C" fn k_comp(mut e: i32, mut alpha: i32, mut gamma: i32) -> i32 {
    ((alpha - e + 63 as i32) as f64 * 0.30102999566398114f64).ceil() as i32
}
...
unsafe extern "C" fn multiply(mut x: diy_fp_t, mut y: diy_fp_t) -> diy_fp_t {
    ...
    let mut tmp: u64 = 0;
    let mut r: diy_fp_t = diy_fp_t { f: 0, e: 0 };
    let mut M32: u64 = 0xffffffff as libc::c_uint as u64;
    a = x.f >> 32;
    b = x.f & M32;
    c = y.f >> 32;
    d = y.f & M32;
    ...
    tmp = (bd >> 32 as i32)
        .wrapping_add(ad & M32)
        .wrapping_add(bc & M32);
    tmp = (tmp as u32).wrapping_add(((1 as libc::c_uint) << 31 as i32) as u32) as u64;
    r.f = ac
        .wrapping_add(ad >> 32 as i32)
        .wrapping_add(bc >> 32 as i32)
        .wrapping_add(tmp >> 32 as i32);
    r.e = x.e + y.e + 64 as i32;
    r
}
unsafe extern "C" fn double_to_uint64(mut d: f64) -> u64 {
    let mut n: u64 = 0;
    memcpy(
        &mut n as *mut u64 as *mut libc::c_void,
        &mut d as *mut f64 as *const libc::c_void,
        8,
    );
    n
}
unsafe extern "C" fn double2diy_fp(mut d: f64) -> diy_fp_t {
    let mut d64: u64 = double_to_uint64(d);
    let mut biased_e: i32 = ((d64 & 0x7ff0000000000000) >> 52) as i32;
    let mut significand: u64 = d64 & 0xfffffffffffff;
    let mut res: diy_fp_t = diy_fp_t { f: 0, e: 0 };
    if biased_e != 0 {
        res.f = significand.wrapping_add(0x10000000000000);
        res.e = biased_e - (0x3ff as i32 + 52 as i32);
    } else {
        res.f = significand;
        res.e = -(0x3ff as i32 + 52 as i32) + 1;
    }
    res
}
unsafe extern "C" fn normalize_boundary(mut in_0: diy_fp_t) -> diy_fp_t {
    let mut res: diy_fp_t = in_0;
    while res.f & ((0x10000000000000) << 1) == 0 {
        res.f <<= 1;
        res.e -= 1;
        res.e;
    }
    res.f <<= 64 as i32 - 52 as i32 - 2;
    res.e -= 64 as i32 - 52 as i32 - 2;
    res
}
unsafe extern "C" fn normalized_boundaries(
    ...
    let mut significand_is_zero: i32 = (v.f == 0x10000000000000) as i32;
    pl.f = (v.f << 1).wrapping_add(1);
    pl.e = v.e - 1;
    pl = normalize_boundary(pl);
    if significand_is_zero != 0 {
        mi.f = (v.f << 2).wrapping_sub(1);
        mi.e = v.e - 2;
    } else {
        mi.f = (v.f << 1).wrapping_sub(1);
        mi.e = v.e - 1;
    }
    mi.f <<= mi.e - pl.e;
    mi.e = pl.e;
...
unsafe extern "C" fn digit_gen(
    ...
    one.f = (1 as i32 as u64) << -Mp.e;
    one.e = Mp.e;
    p1 = (Mp.f >> -one.e) as uint32_t;
    p2 = Mp.f & (one.f).wrapping_sub(1);
    *len = 0;
    kappa = 3;
    div = 100 as uint32_t;
    while kappa > 0 {
        d = p1.wrapping_div(div) as i32;
        if d != 0 || *len != 0 {
            let fresh23 = *len;
            *len += 1;
        ...
        div = (div as libc::c_uint).wrapping_div(10 as libc::c_uint) as uint32_t as uint32_t;
        if ((p1 as u64) << -one.e).wrapping_add(p2) <= delta.f {
            *K += kappa;
            return;
        }
    }
    loop {
        p2 = (p2 as u32).wrapping_mul(10) as u64 as u64;
        d = (p2 >> -one.e) as i32;
        if d != 0 || *len != 0 {
            let fresh24 = *len;
            *len += 1;
        ...
        p2 &= (one.f).wrapping_sub(1);
        kappa -= 1;

        delta.f = (delta.f as u32).wrapping_mul(10) as u64;
        if p2 <= delta.f {
            break;
        }
    }
...
pub unsafe extern "C" fn js_grisu2(
    ...
    let mut q: i32 = 64 as i32;
    let mut alpha: i32 = -(59 as i32);
    let mut gamma: i32 = -(56 as i32);
...
pub unsafe extern "C" fn js_strtod(
    ...
    let mut sign: i32 = 0;
    let mut expSign: i32 = 0;
    let mut fraction: f64 = 0.;
    let mut dblExp: f64 = 0.;
    let mut d: *const f64 = core::ptr::null::<f64>();
    let mut p: *const libc::c_char = core::ptr::null::<libc::c_char>();
    let mut c: i32 = 0;
    let mut exp_0: i32 = 0;
    let mut fracExp: i32 = 0;
    let mut mantSize: i32 = 0;
    let mut decPt: i32 = 0;
    let mut pExp: *const libc::c_char = core::ptr::null::<libc::c_char>();
    p = string;
    ...
    if mantSize > 18 as i32 {
        fracExp = decPt - 18 as i32;
        mantSize = 18 as i32;
    } else {
    ...
    if mantSize == 0 {
        ...
        while mantSize > 0 {
            ...
            frac2 = 10 * frac2 + (c - '0' as i32);
            mantSize -= 1;
        }
        fraction = 1.0e9f64 * frac1 as f64 + frac2 as f64;
        p = pExp;
        if *p as i32 == 'E' as i32 || *p as i32 == 'e' as i32 {
            p = p.offset(1 as i32 as isize);
            if *p as i32 == '-' as i32 {
                expSign = 1;
                p = p.offset(1 as i32 as isize);
            ...
            while *p as i32 >= '0' as i32
                && *p as i32 <= '9' as i32
                && exp_0 < 2147483647 as i32 / 100
            {
                exp_0 = exp_0 * 10 + (*p as i32 - '0' as i32);
                p = p.offset(1 as i32 as isize);
            }
        ...
        if exp_0 < -MAX_EXPONENT {
            exp_0 = MAX_EXPONENT;
            expSign = 1;
            *__errno_location() = 34 as i32;
        } else if exp_0 > MAX_EXPONENT {
            exp_0 = MAX_EXPONENT;
            expSign = 0;
            *__errno_location() = 34 as i32;
        } else if exp_0 < 0 {
        ...
        dblExp = 1.0f64;
        d = POWERS_OF_10.as_ptr();
        while exp_0 != 0 {
            if exp_0 & 0o1 as i32 != 0 {
                dblExp *= *d;
            }
            exp_0 >>= 1;
            d = d.offset(1 as i32 as isize);
        }
        if expSign != 0 {
            fraction /= dblExp;
        } else {
